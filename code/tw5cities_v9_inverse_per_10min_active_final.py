...
def plot_grey_level_image(data,time_period,y_title,title):
    ...
    #print "image 1300",image_array[:,1239]
    aggreated_result=image_array[:,1220]
    #print aggreated_result
    for k in range(1221,1270):
        aggreated_result+=image_array[:,k]
    #print aggreated_result/(float(sum(aggreated_result)))
        
    return aggreated_result/(float(sum(aggreated_result)))
...
def  visualize_city_one_by_one(data_array,city_name):
    ...
    color_list=['#ca0020', '#f4a582', '#ffffff','#bababa','#404040']
    title='all_cities'
    #time_period=60*24
    #every 10 minutes.
    time_period=2*24
    ...
    for i in range(len(city_name)):
        ...
        title=city_name[i]
        #label_name=title
        x_time=np.arange(time_period)/2.0
        x=[]
        for k in range(len(x_time)):
            x.append(str(datetime.timedelta(hours=x_time[k])))
        print "x",x
        x_label=np.array(x)
        y=plot_grey_level_image(data_array[data_array[:,0]==city_name[i]],time_period,y_title,title)
        #y.tolist()
        print "y",y
        plt.bar(x_time, y,color=color_list[i])
        #plt.set_xtricks(x_time)
        #plt.set_xtricklabels(x_label,rotation=90)
        plt.title(title)
    plt.savefig('/home/tingxu/Documents/jimmy/tw5cities/output/'+title+'final.png')
    plt.show()
