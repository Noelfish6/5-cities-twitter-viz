...
def plot_hist(image,title):
    hist, bin_edges = np.histogram(image, bins=256, range=(0.0, 255), density=False)
    plt.figure(1)
    #plt.subplot(311)
...
def plot_grey_level_image(data,time_period,y_title,title):
    ...
    binary_image=assignment2_p1.get_binary_image(image_array,threshold=125,low_threshold_indi=False,title=title)
